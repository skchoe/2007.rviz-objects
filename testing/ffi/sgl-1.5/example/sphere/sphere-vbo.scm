(module sphere-vbo scheme
  ...
  (define (define-sphere-indices num_row num_col)
    (let* ([idx-list (build-list 0 (lambda (x) 0))])
      ...
      (let ([cv (list->cvector idx-list _gl-uint)])
        (values cv (cvector-type cv) (length idx-list)))
      ))
  ...
  (define-values (index-set _idx-type num-idx) 
    (let-values ([(pt ty nm) (define-sphere-indices num_row num_col)])
      (cvector->pointer pt ty nm)))
